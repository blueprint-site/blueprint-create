...
module.exports = async function (context) {
  ...
  const apiKey =
    process.env.APPWRITE_FUNCTION_API_KEY || process.env.APPWRITE_API_KEY || process.env.API_KEY;
  ...
  const client = new sdk.Client()
    .setEndpoint(process.env.APPWRITE_ENDPOINT || 'https://api.blueprint-create.com/v1')
    .setProject(process.env.APPWRITE_FUNCTION_PROJECT_ID || '683770670016d1661c37');
  ...
  log('SDK initialized with:', {
    endpoint: process.env.APPWRITE_ENDPOINT || 'https://api.blueprint-create.com/v1',
    projectId: process.env.APPWRITE_FUNCTION_PROJECT_ID || '683770670016d1661c37',
    hasApiKey: !!apiKey,
  });
  ...
  try {
    ...
    if (jwt) {
      log('Request includes JWT for audit trail - using function API key for operations');
    } else {
    ...
    switch (action) {
      ...
      case 'createUser': {
        ...
        if (!email || !password || !name) {
          return res.json({ success: false, error: 'email, password, and name are required' }, 400);
        }
      ...
      case 'updateUserStatus': {
        ...
        if (!userId || status === undefined) {
          return res.json({ success: false, error: 'userId and status are required' }, 400);
        }
      ...
      case 'updateUserLabels': {
        ...
        if (!userId || !Array.isArray(labels)) {
          return res.json({ success: false, error: 'userId and labels array are required' }, 400);
        }
      ...
      case 'updateEmailVerification': {
        ...
        if (!userId || emailVerification === undefined) {
          return res.json(
            { success: false, error: 'userId and emailVerification are required' },
            400
          );
        }
      ...
      case 'updateTeamMembership': {
        ...
        if (!userId || !teamId || add === undefined) {
          return res.json({ success: false, error: 'userId, teamId, and add are required' }, 400);
        }
