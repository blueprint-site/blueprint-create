          ...
          if (isNbt) {
            ...
            if (!dryRun) {
              try {
                ...
                const fileData = await storage.getFileDownload(sourceBucketId, file.$id);
...
async function updateSchematicReferences(databases, fileId, oldBucketId, newBucketId, field, log) {
  try {
    ...
    while (hasMore) {
      ...
      for (const doc of docs.documents) {
        ...
        if (field === 'schematic_url' && doc.schematic_url && doc.schematic_url.includes(fileId)) {
          // Update single URL
          ...
          needsUpdate = true;
        } else if (field === 'image_urls' && doc.image_urls && Array.isArray(doc.image_urls)) {
          // Update array of URLs
          const updatedUrls = doc.image_urls.map((url) => {
            if (url && url.includes(fileId)) {
              return url.replace(`/buckets/${oldBucketId}/`, `/buckets/${newBucketId}/`);
            }
            return url;
          });
        ...
        if (needsUpdate) {
          await databases.updateDocument('main', 'schematics', doc.$id, updates);
          updatedCount++;
