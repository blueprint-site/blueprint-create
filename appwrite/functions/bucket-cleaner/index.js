...
module.exports = async ({ req, res, log, error }) => {
  try {
    ...
    for (const bucketId of buckets) {
      ...
      try {
        const referencedFiles = await getReferencedFiles(databases, bucketId, log);
        log(`Found ${referencedFiles.size} referenced files in bucket ${bucketId}`);
        ...
        while (hasMore) {
          try {
            ...
            for (const file of files.files) {
              ...
              if (!referencedFiles.has(fileIdentifier) && !referencedFiles.has(file.$id)) {
                ...
                if (!dryRun) {
                  try {
                    ...
                    log(`Deleted orphaned file: ${file.$id} from bucket ${bucketId}`);
                  } catch (deleteErr) {
...
async function getReferencedFiles(databases, bucketId, log) {
  ...
  const collectionMappings = {
    'schematics-nbt': {
      collections: [{ db: 'main', collection: 'schematics', fields: ['schematic_url'] }],
    },
    'schematics-previews': {
      collections: [{ db: 'main', collection: 'schematics', fields: ['image_urls'] }],
    },
  ...
  for (const config of mapping.collections) {
    try {
      ...
      while (hasMore) {
        const docs = await databases.listDocuments(config.db, config.collection, [
          sdk.Query.limit(100),
          sdk.Query.offset(offset),
        ]);
      ...
      log(`Error fetching documents from ${config.collection}: ${collErr.message}`);
    }
  }
...
function extractFileIdForBucket(url, targetBucketId) {
  ...
  if (url.includes(`/buckets/${targetBucketId}/files/`)) {
    ...
    const match = url.match(new RegExp(`/buckets/${targetBucketId}/files/([^/]+)/`));
    if (match && match[1]) {
...
async function getUserAvatars(databases, log) {
  ...
  try {
    ...
    while (hasMore) {
      const userList = await users.list([sdk.Query.limit(100), sdk.Query.offset(offset)]);
      ...
      for (const user of userList.users) {
        ...
        if (user.prefs && user.prefs.avatar) {
          const avatarId = extractFileIdForBucket(user.prefs.avatar, '67aee2b30000b9e21407');
          if (avatarId) {
