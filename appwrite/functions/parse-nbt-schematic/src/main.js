...
function calculateComplexity(dimensions, uniqueBlockTypes, hasRedstone, modCount) {
  let score = 0;
  ...
  try {
    ...
    if (rawData.palette) {
      ...
      console.log(
        ...
        rawData.palette.value.value.slice(0, 3).map((entry) => entry.Name?.value)
      );
    }
    if (rawData.blocks) {
      console.log('Found blocks array, length:', rawData.blocks.value.value.length);
      console.log('First 10 block entries:', rawData.blocks.value.value.slice(0, 10));
    }
    if (rawData.size) {
    ...
    if ((schematic.Palette || schematic.palette) && (schematic.BlockData || schematic.blockData)) {
      ...
      console.log(`BlockData type: ${typeof blockData}, is array: ${Array.isArray(blockData)}`);
      if (blockData && blockData.length) {
        ...
        for (let i = 0; i < blockData.length; i++) {
          ...
          if (blockId && blockId !== 'minecraft:air' && blockId !== 'air') {
            ...
            if (!blockMap.has(key)) {
              const modName =
                namespace !== VANILLA_NAMESPACE ? MOD_MAPPING[namespace] || namespace : 'Minecraft';
              blockMap.set(key, {
                id: key,
                name: cleanBlockName,
    ...
    // Format 1b: Raw NBT format with palette and blocks arrays
      ...
      if (blocks.value && Array.isArray(blocks.value.value)) {
        console.log(`Processing NBT blocks array with ${blocks.value.value.length} entries`);
        ...
        console.log(`Processed ${processedBlocks} non-air blocks from NBT blocks array`);
      }
    }
    ...
    const primaryMaterials = blocks
      ...
      .map((block) => ({
        ...
        count: block.count,
        percentage:
          dimensions.blockCount > 0 ? Math.round((block.count / dimensions.blockCount) * 100) : 0,
      }));
