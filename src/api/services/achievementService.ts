...
export class AchievementService {
   ...
   */
  static async trackContentUpload(userId: string, contentType: 'schematic' | 'addon'): Promise<void> {
    try {
   ...
   */
  static async trackContentDownload(downloaderId: string, contentOwnerId: string, contentId: string): Promise<void> {
    try {
   ...
   */
  static async trackLike(likerId: string, contentOwnerId: string, contentId: string): Promise<void> {
    try {
      await Promise.all([
   ...
   */
  static async trackProfileCompletion(userId: string, profileData: Record<string, unknown>): Promise<void> {
    try {
   ...
   */
  static async applyRuleRetroactively(ruleId: string, fromDate?: string): Promise<{ processed: number; awarded: number }> {
    try {
      ...
      for (const user of usersResponse.documents) {
        try {
          const result = await RewardRulesEngine.checkSingleRule(user.$id, parsedRule as unknown as RewardRule);
          processedCount++;
      ...
      toast.success(`Retroactive processing complete: ${awardedCount} badges awarded to ${processedCount} users`);
   ...
   */
  static async getAchievementRecommendations(userId: string): Promise<Array<{
    ruleId: string;
    badgeId: string;
    badgeName: string;
    progress: number;
    ...
    try {
      ...
      for (const ruleDoc of rulesResponse.documents) {
        ...
        try {
          ...
          const result = await RewardRulesEngine.checkSingleRule(userId, parsedRule as unknown as RewardRule);
          ...
          if (!result.awarded && result.progressUpdated) {
            ...
            if (testResult.documents.length > 0) {
              ...
              if (percentage >= 25 && percentage < 100) {
                const badge = await databases.getDocument(DATABASE_ID, 'badges', parsedRule.badgeId);
                ...
                recommendations.push({
                  ...
                  suggestion: this.generateProgressSuggestion(parsedRule as unknown as RewardRule, progress as unknown as { currentValue: number; targetValue: number }),
                });
              }
            }
          }
   ...
   */
  private static generateProgressSuggestion(rule: RewardRule, progress: { currentValue: number; targetValue: number }): string {
