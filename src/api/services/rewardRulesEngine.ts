...
export class RewardRulesEngine {
   ...
   */
  static async checkAchievementsForEvent(userId: string, eventType: string, eventData?: Record<string, unknown>): Promise<AwardResult[]> {
    try {
  ...
  private static evaluateMilestoneConditions(
    ...
    const progress = operator === '>=' || operator === '>' 
      ? Math.min((currentValue / targetValue) * 100, 100)
      : meetsConditions ? 100 : 0;
  ...
  private static evaluateActionConditions(
    ...
    switch (actionType) {
      ...
      case 'first_upload':
        meetsConditions = (userStats.totalSchematics || 0) >= 1 || (userStats.totalAddons || 0) >= 1;
  ...
  private static async updateProgressTracking(
    ...
    try {
      ...
      const existingResponse = await databases.listDocuments(DATABASE_ID, ACHIEVEMENT_PROGRESS_COLLECTION_ID, [
        Query.equal('userId', userId),
      ...
      const now = new Date().toISOString();
      const percentage = Math.min(Math.round((evaluation.currentValue / evaluation.targetValue) * 100), 100);
      ...
      if (existingResponse.documents.length > 0) {
        ...
        if (existing.currentValue !== evaluation.currentValue || 
  ...
  private static async getExistingAchievement(userId: string, ruleId: string): Promise<UserAchievement | null> {
    try {
  ...
  private static async getExistingAchievementWithLock(userId: string, ruleId: string): Promise<UserAchievement | null> {
  ...
  private static checkCooldown(achievement: UserAchievement, cooldownDays: number): { canEarn: boolean; nextAvailableDate?: string } {
    if (cooldownDays === 0) {
      return { canEarn: true };
    }
  ...
  private static filterRulesForEvent(rules: RewardRule[], eventType: string): RewardRule[] {
    ...
    return rules.filter(rule => {
      switch (eventType) {
        ...
        case 'follow':
          return rule.conditions.metric?.includes('followers') ||
