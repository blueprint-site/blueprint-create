...
export const useUserAchievementProgress = (userId?: string, ruleId?: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await databases.listDocuments(DATABASE_ID, ACHIEVEMENT_PROGRESS_COLLECTION_ID, queries);
      return response.documents.map((doc) => AchievementProgressSchema.parse(doc)) as AchievementProgress[];
    },
    enabled: !!userId,
  });
};
...
export const useAchievementSummary = (userId?: string) => {
  return useQuery({
    ...
    queryFn: async (): Promise<AchievementSummary | null> => {
      ...
      const achievementsResponse = await databases.listDocuments(DATABASE_ID, USER_ACHIEVEMENTS_COLLECTION_ID, [
        Query.equal('userId', userId),
        Query.orderDesc('awardedAt'),
        Query.limit(10),
      ]);
      ...
      const progressResponse = await databases.listDocuments(DATABASE_ID, ACHIEVEMENT_PROGRESS_COLLECTION_ID, [
        Query.equal('userId', userId),
      ...
      const nextBadges = await Promise.all(
        progress.map(async (p) => {
          try {
            const ruleDoc = await databases.getDocument(DATABASE_ID, REWARD_RULES_COLLECTION_ID, p.ruleId);
            const badge = await databases.getDocument(DATABASE_ID, BADGES_COLLECTION_ID, ruleDoc.badgeId);
...
export const useUpdateUserAchievement = () => {
  ...
  return useMutation({
    mutationFn: async ({ achievementId, updates }: { achievementId: string; updates: UpdateUserAchievement }) => {
...
export const useRevokeAchievement = () => {
  ...
  return useMutation({
    mutationFn: async (achievementId: string) => {
      const achievement = await databases.getDocument(DATABASE_ID, USER_ACHIEVEMENTS_COLLECTION_ID, achievementId);
...
export const useUpdateAchievementProgress = () => {
  ...
  return useMutation({
    mutationFn: async ({ progressId, updates }: { progressId: string; updates: UpdateAchievementProgress }) => {
      const response = await databases.updateDocument(
        DATABASE_ID,
        ACHIEVEMENT_PROGRESS_COLLECTION_ID,
        progressId,
...
export const usePinAchievement = () => {
  ...
  return useMutation({
    mutationFn: async ({ achievementId, isPinned }: { achievementId: string; isPinned: boolean }) => {
      const response = await databases.updateDocument(
        DATABASE_ID,
        USER_ACHIEVEMENTS_COLLECTION_ID,
        achievementId,
