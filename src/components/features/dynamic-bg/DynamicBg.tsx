...
export const DynamicBg = ({
    ...
    const createIconGrid = () => {
        ...
        for (let row = 0; row < rows; row++) {
            ...
            for (let col = 0; col < cols * 2; col++) { // double the columns for seamless scrolling
    ...
    return (
        <div className="relative h-full w-full overflow-hidden rounded-b-2xl" ref={containerRef}>
            ...
            {/* Main content */}
            <div className="relative flex h-full flex-col items-center justify-center text-white font-bold">
                <h1 className="text-4xl mb-4">{title || "0% of addons have support for Create 0.6"}</h1>
                <p className="text-xl italic">{subtitle || "\"That accounts to ~0 addons totally\" - Says our expert"}</p>
            </div>
            <div className="absolute top-2 right-2  flex items-center">
                <div className="relative group mr-1">
                    <Info className="h-4 w-4 cursor-pointer" />
                    <div className="absolute hidden group-hover:block bg-gray-800 text-white text-sm p-2 rounded shadow-lg z-10 right-full mr-2">
