...
export const ProfileRewardsTab: React.FC = () => {
  ...
  const { data: achievements, isLoading: achievementsLoading, refetch: refetchAchievements } = useUserAchievements(user?.$id);
  ...
  const isLoading = statsLoading || achievementsLoading || badgesLoading || rulesLoading || progressLoading;
  ...
  const achievementWithBadges = useMemo(() => {
    ...
    return userAchievements.map(achievement => {
      ...
      if (!badge && achievement.badgeId) {
        console.warn(`Badge not found for achievement ${achievement.$id} with badgeId ${achievement.badgeId}`);
      }
      return {
        ...achievement,
        badge
  ...
  const availableAchievements = useMemo(() => {
    ...
    const activeRules = rules.filter(rule => 
      rule.isActive && 
      !rule.testMode &&
      (!earnedRuleIds.includes(rule.$id) || rule.isRepeatable)
    );
        ...
        <CardContent>
          {achievementWithBadges.length > 0 ? (
              ...
              {achievementWithBadges.map((achievement) => (
                <div key={achievement.$id} className='relative'>
                  ...
                  <div className={`p-4 rounded-lg border-2 transition-all ${
                    achievement.badge?.tier === 'legendary' ? 'border-yellow-500 bg-yellow-50 dark:bg-yellow-950' :
                    achievement.badge?.tier === 'epic' ? 'border-purple-500 bg-purple-50 dark:bg-purple-950' :
                    achievement.badge?.tier === 'rare' ? 'border-blue-500 bg-blue-50 dark:bg-blue-950' :
                    ...
                    <div className='flex items-start gap-3'>
                      ...
                      <div className='flex-1'>
                        <h3 className='text-sm font-semibold'>
                          {achievement.badge?.name || `Achievement ${achievement.$id?.substring(0, 8)}`}
                        </h3>
                        <p className='text-sm text-muted-foreground mt-1'>
                          {achievement.badge?.description || 'Badge not found - ID: ' + achievement.badgeId}
                        </p>
                        {achievement.reason && (
            ...
            <div className='text-center py-8'>
              ...
              <p className='text-sm text-muted-foreground mt-2'>Complete challenges to earn badges!</p>
            </div>
          )}
        </CardContent>
          </Card>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                  {availableAchievements.map((rule) => (
                    <div key={rule.$id} className='relative'>
                      ...
                      <div className={`p-4 rounded-lg border-2 transition-all ${
                        rule.badge?.tier === 'legendary' ? 'border-yellow-300 bg-yellow-50/50 dark:bg-yellow-950/50' :
                        rule.badge?.tier === 'epic' ? 'border-purple-300 bg-purple-50/50 dark:bg-purple-950/50' :
                        rule.badge?.tier === 'rare' ? 'border-blue-300 bg-blue-50/50 dark:bg-blue-950/50' :
                        ...
                        <div className='flex items-start gap-3'>
                          ...
                          <div className='flex-1'>
                            ...
                            <p className='text-sm text-muted-foreground mt-1'>
                              {rule.badge?.description || rule.description || 'Badge ID: ' + rule.badgeId}
                            </p>
                            ...
                            {rule.progressVisible && (
                              <div className='mt-3'>
                                ...
                                <Progress 
                                  value={rule.progress 
                                    ? (rule.progress.currentValue / rule.progress.targetValue) * 100
                                    : 0
                                  } 
                                  className='h-2' 
                                />
      ...
      <Card>
        ...
        <CardContent>
          <div className='space-y-4'>
            ...
            <div className='flex justify-between items-center'>
              <span className='text-sm text-muted-foreground'>Available Challenges</span>
              <span className='font-semibold'>
                {availableAchievements.filter(a => !a.isEarned).length}
              </span>
            </div>
            <div className='flex justify-between items-center'>
              <span className='text-sm text-muted-foreground'>Total Downloads Received</span>
