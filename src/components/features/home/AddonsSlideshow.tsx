...
const AddonsCarousel = () => {
  ...
  const addons = [
    {
      ...
      banner: 'https://cdn.modrinth.com/data/Dq3STxps/images/f6486f7c9d0b2aee956402a864af9347c607ee0a.png',
    ...
    },
    {
      ...
      banner: 'https://cdn.modrinth.com/data/ZzjhlDgM/images/b541ced05f30da9024e30f28d3cd83520bb1a45f.webp',
  ...
  return (
    <div className='mx-auto flex h-full max-w-6xl items-center justify-center gap-4'>
      <Button onClick={() => scrollToIndex(current - 1)} className='cursor-pointer rounded-full p-0' variant='icon'>
        <MinecraftIcon name='chevron-left' size={32} />
      </Button>

      <div className='relative flex-1'>
        <Carousel setApi={setApi} opts={{ align: 'end', loop: true }} plugins={[Autoplay({ delay: 5000 })]}>
          <CarouselContent>
            {addons.map((addon, index) => (
              <CarouselItem key={index}>
                <img loading='lazy' src={addon.banner} alt='' className='h-96 max-h-full rounded-lg' />
              </CarouselItem>
            ))}
          </CarouselContent>
        </Carousel>
      ...
      <div className='h-96 w-80'>
        <Card className='bg-background h-full'>
          <CardHeader>
            <CardTitle className='text-center text-2xl underline'>{addons[current].title}</CardTitle>
          </CardHeader>
          <CardContent className='flex flex-col items-center justify-center p-4'>
            <img loading='lazy' className='h-full w-auto object-contain' src={addons[current].image} alt='' />
