...
export const AchievementNotificationProvider: React.FC<AchievementNotificationProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    unsubscribeRef.current = client.subscribe(channel, async (response: RealtimeResponseEvent<UserAchievement>) => {
      // Check if this is a create event for the current user
      if (
        response.events.includes('databases.main.collections.user_achievements.documents.*.create') &&
      ...
      try {
        ...
        if (response.documents.length > 0) {
          ...
          if (newAchievements.length > 0) {
            const achievementsWithBadges = newAchievements.map((achievement) => {
              const badge = badges?.find((b) => b.$id === achievement.badgeId);
              return {
                ...achievement,
                badge,
