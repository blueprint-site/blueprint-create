  ...
  res: {
    json: (data, status = 200) => {
      ...
      if (data.success && data.results) {
        ...
        console.log("\n=== Per-Bucket Results ===");
        for (const [bucketId, bucketData] of Object.entries(
          data.results.buckets,
        )) {
          console.log(`\nBucket: ${bucketId}`);
          console.log(`  Files Checked: ${bucketData.filesChecked}`);
          console.log(`  Orphaned Files: ${bucketData.orphaned.length}`);
          if (
            bucketData.orphaned.length > 0 &&
            bucketData.orphaned.length <= 10
          ) {
            bucketData.orphaned.forEach((file) => {
              console.log(
                `    - ${file.name} (${file.size} bytes, created: ${file.created})`,
              );
            });
          } else if (bucketData.orphaned.length > 10) {
            console.log(
              `    (Showing first 10 of ${bucketData.orphaned.length} files)`,
            );
...
process.env.APPWRITE_FUNCTION_API_ENDPOINT =
  process.env.APPWRITE_FUNCTION_API_ENDPOINT ||
  "https://api.blueprint-create.com/v1";
