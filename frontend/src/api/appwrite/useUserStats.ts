...
import { UserStatsSchema, type UserStats, type UpdateUserStats } from '@/schemas/userStats.schema';
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (response.documents.length === 0) {
          ...
          const newStats = await databases.createDocument(DATABASE_ID, COLLECTION_ID, ID.unique(), {
            userId,
          });
...
export const useUpdateUserStats = () => {
  ...
  return useMutation({
    mutationFn: async ({ statsId, updates }: { statsId: string; updates: UpdateUserStats }) => {
      const response = await databases.updateDocument(DATABASE_ID, COLLECTION_ID, statsId, updates);
...
export const useIncrementUserStat = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // Update the field
      const response = await databases.updateDocument(DATABASE_ID, COLLECTION_ID, stats.$id, {
