      ...
      <CardContent>
        ...
        {trend !== undefined && (
          <div className='mt-1 flex items-center text-xs text-green-500'>
            <TrendingUp className='mr-1 h-3 w-3' />+{trend}% from last month
          </div>
        )}
      </CardContent>
    </Card>
...
export const GlobalStats = () => {
      ...
      <div className='space-y-6'>
        <div>
          ...
          <p className='text-muted-foreground'>
            Platform-wide user activity and engagement metrics
          </p>
        </div>
      ...
      <div className='space-y-6'>
        ...
        <Card>
          <CardContent className='pt-6'>
            <p className='text-muted-foreground text-center'>No user statistics available yet.</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  const totalDownloads = allUserStats.reduce(
    (sum, stat) => sum + (stat.totalDownloadsReceived || 0),
    0
  );
  ...
  const totalRatings = allUserStats.reduce(
    (sum, stat) => sum + (stat.totalRatingsReceived || 0),
    0
  );
  const averageRating =
    totalRatings > 0
  ...
  const activeUsers = allUserStats.filter(
    (stat) => stat.lastLoginAt && new Date(stat.lastLoginAt) > thirtyDaysAgo
  ).length;

  // Top performers
  const topDownloadUser = allUserStats.reduce(
    (max, stat) =>
      (stat.totalDownloadsReceived || 0) > (max.totalDownloadsReceived || 0) ? stat : max,
    allUserStats[0]
  );

  const topContentCreator = allUserStats.reduce(
    (max, stat) =>
      (stat.totalSchematics || 0) + (stat.totalAddons || 0) >
      (max.totalSchematics || 0) + (max.totalAddons || 0)
        ? stat
        : max,
    allUserStats[0]
  );
  ...
  return (
    <div className='space-y-6'>
      ...
      {/* Top Performers */}
      <div>
        <h2 className='mb-4 text-xl font-semibold'>Top Performers</h2>
            ...
            <CardContent>
              <div className='space-y-2'>
                ...
                <div className='text-muted-foreground text-xs'>
                  {topDownloadUser.totalSchematics || 0} schematics •{' '}
                  {topDownloadUser.totalAddons || 0} addons
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className='space-y-2'>
                ...
                <p className='text-muted-foreground text-sm'>
                  {(
                    (topContentCreator.totalSchematics || 0) + (topContentCreator.totalAddons || 0)
                  ).toLocaleString()}{' '}
                  total content pieces
                </p>
                <div className='text-muted-foreground text-xs'>
                  {topContentCreator.totalSchematics || 0} schematics •{' '}
                  {topContentCreator.totalAddons || 0} addons
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      {/* Recent Milestones */}
      <div>
        <h2 className='mb-4 text-xl font-semibold'>Recent Milestones</h2>
        <Card>
          <CardContent className='pt-6'>
            <div className='space-y-4'>
              ...
              {allUserStats.filter((stat) => stat.reached100Downloads || stat.reached1000Downloads)
                .length === 0 && (
                <p className='text-muted-foreground text-center'>No milestones reached yet.</p>
              )}
            </div>
          </CardContent>
        </Card>
