...
export const BadgeDebug = () => {
  ...
  useEffect(() => {
    ...
    rules.forEach((rule) => {
      ...
      if (!badge) {
        newIssues.push(
          `Rule "${rule.name}" (${rule.$id}) references missing badge: ${rule.badgeId}`
        );
      } else {
    ...
    achievements?.forEach((achievement) => {
      ...
      if (!badge) {
        newIssues.push(
          `Achievement ${achievement.$id} references missing badge: ${achievement.badgeId}`
        );
      }
    });
    ...
    badges.forEach((badge) => {
      ...
      if (!badge.iconUrl) {
        ...
        newValidations.push(
          `✓ Badge "${badge.name}" has iconUrl: ${badge.iconUrl.substring(0, 50)}...`
        );
      }
    });
  ...
  return (
    <div className='flex-1 space-y-6 p-4 pt-6 md:p-8'>
      ...
      <div className='grid gap-4'>
        <Card>
          <CardHeader>
            <CardTitle className='flex items-center gap-2'>System Overview</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className='space-y-4'>
              {badges?.map((badge) => (
                <div key={badge.$id} className='rounded-lg border p-3'>
                  ...
                  <div className='text-muted-foreground text-xs'>ID: {badge.$id}</div>
                  <div className='text-xs'>Icon URL: {badge.iconUrl ? '✓ Set' : '✗ Missing'}</div>
