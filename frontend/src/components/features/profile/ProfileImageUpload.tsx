...
export default function ProfileImageUpload({
  ...
  const handleImageUpload = useCallback(
    async (file: File) => {
      ...
      logMessage(
        `🔍 DEBUG: Starting upload with currentAvatar prop: "${currentAvatar}"`,
        0,
        'debug'
      );
      ...
      logMessage(
        `🔍 DEBUG: avatarUrlToCheck includes 'appwrite': ${avatarUrlToCheck?.includes('appwrite')}`,
        0,
        'debug'
      );
      logMessage(
        `🔍 DEBUG: avatarUrlToCheck includes 'api.blueprint-create.com': ${avatarUrlToCheck?.includes('api.blueprint-create.com')}`,
        0,
        'debug'
      );
      ...
      if (isAppwriteUrl) {
        ...
        for (let i = 0; i < patterns.length; i++) {
          ...
          logMessage(
            `Pattern ${i + 1}: ${pattern.source} - Match: ${match ? match[1] : 'no match'}`,
            0,
            'debug'
          );
          if (match && match[1]) {
            oldAvatarId = match[1];
            logMessage(
              `✓ Extracted old avatar ID: ${oldAvatarId} using pattern ${i + 1}`,
              0,
              'action'
            );
            break;
      ...
      try {
        ...
        const uploadedFile = await storage.createFile(STORAGE_BUCKETS.AVATARS, fileId, file);
        ...
        if (result.uploadFailed && result.compressed.base64) {
          logMessage(
            'Compression succeeded but upload failed, uploading from client...',
            1,
            'action'
          );
        ...
        const avatarUrl = storage
          .getFilePreview(STORAGE_BUCKETS.AVATARS, compressedFileId)
          .toString();
        ...
        logMessage(
          `🔍 DEBUG: About to check for old avatar deletion. oldAvatarId: "${oldAvatarId}"`,
          0,
          'debug'
        );
        ...
        if (oldAvatarId) {
          if (oldAvatarId === compressedFileId) {
            logMessage(
              `Skipping deletion - old and new IDs are the same: ${oldAvatarId}`,
              1,
              'action'
            );
          } else {
            logMessage(
              `🗑️ Attempting to delete old avatar: ${oldAvatarId} from bucket: ${STORAGE_BUCKETS.AVATARS}`,
              0,
              'action'
            );
            ...
            try {
              ...
              logMessage(
                `✓ Old avatar file exists: ${existingFile.name} (${existingFile.sizeOriginal} bytes)`,
                0,
                'action'
              );
              ...
              logMessage(
                `ℹ️ Old avatar file doesn't exist or can't be accessed: ${errorMessage}`,
                1,
                'action'
              );
            ...
            try {
              logMessage(
                `🔥 Executing deletion: storage.deleteFile("${STORAGE_BUCKETS.AVATARS}", "${oldAvatarId}")`,
                0,
                'debug'
              );
              ...
              logMessage(
                `❌ Failed to delete old avatar ${oldAvatarId}: ${errorMessage} (code: ${errorCode}, type: ${errorType})`,
                3,
                'action'
              );
              ...
              if (
                ...
                logMessage(
                  `ℹ️ Old avatar ${oldAvatarId} was already deleted or doesn't exist`,
                  1,
                  'action'
                );
                ...
                toast.error(`Could not remove previous avatar: ${errorMessage}`, {
                  duration: 5000,
                });
              }
            }
          ...
          logMessage(
            `🔍 DEBUG: Why no deletion? avatarUrlToCheck: "${avatarUrlToCheck}"`,
            0,
            'debug'
          );
          logMessage(`🔍 DEBUG: isAppwriteUrl: ${isAppwriteUrl}`, 0, 'debug');
        ...
        if (isCurrentAppwriteUrl) {
          ...
          if (oldFileId) {
            try {
              ...
              logMessage(
                `✓ Deleted old avatar after setting Minecraft skin: ${oldFileId}`,
                0,
                'action'
              );
            } catch (deleteError: unknown) {
              ...
              if (errorMessage.includes('404') || errorMessage.includes('not found')) {
                logMessage(
                  `Old avatar ${oldFileId} was already deleted or doesn't exist`,
                  1,
                  'action'
                );
              } else {
            ...
            logMessage(
              `❌ Could not extract file ID from avatar URL: ${currentAvatar}`,
              1,
              'action'
            );
          }
  ...
  const testAvatarDeletion = useCallback(async (fileId: string) => {
    ...
    try {
      ...
      logMessage(
        `✓ File exists: ${existingFile.name} (${existingFile.sizeOriginal} bytes)`,
        0,
        'debug'
      );
          ...
          {uploadStatus.details && (
            <div className='text-muted-foreground space-y-1 text-xs'>
              {uploadStatus.details.originalSize && (
                <div>
                  Original: {(uploadStatus.details.originalSize / 1024).toFixed(1)} KB → Compressed:{' '}
                  {(uploadStatus.details.compressedSize! / 1024).toFixed(1)} KB
                </div>
              )}
