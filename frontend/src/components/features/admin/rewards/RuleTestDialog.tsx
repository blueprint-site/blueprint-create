...
export const RuleTestDialog: React.FC<RuleTestDialogProps> = ({ isOpen, onClose, rule }) => {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className='max-h-[90vh] max-w-2xl overflow-y-auto'>
        ...
        <div className='space-y-6'>
          ...
          <Card>
            <CardHeader>
              <CardTitle className='flex items-center gap-2 text-lg'>Rule Information</CardTitle>
            </CardHeader>
            <CardContent className='space-y-3'>
              ...
              <div className='space-y-1 text-sm'>
                <div>
                  <strong>Badge:</strong> {ruleBadge?.name || 'Unknown Badge'}
                </div>
                <div>
                ...
                {rule.cooldownDays > 0 && (
                  <div>
                    <strong>Cooldown:</strong> {rule.cooldownDays} days
                  </div>
                )}
              </div>
          ...
          <Card>
            ...
            <CardContent className='space-y-4'>
              <div className='flex items-center gap-4'>
                ...
                <Button onClick={handleTest} disabled={!selectedUserId || testMutation.isPending}>
          ...
          {testResult && (
            <Card>
              ...
              <CardContent className='space-y-4'>
                {/* Overall Result */}
                <div className={`rounded-lg border p-4 ${getResultColor(testResult)}`}>
                ...
                {testResult.currentProgress !== undefined &&
                  testResult.targetValue !== undefined && (
                    <div className='space-y-2'>
                      ...
                      <Progress
                        value={Math.min(
                          (testResult.currentProgress / testResult.targetValue) * 100,
                          100
                        )}
                      />
                      <div className='text-muted-foreground text-xs'>
                        {Math.round((testResult.currentProgress / testResult.targetValue) * 100)}%
                        complete
                      </div>
                    </div>
                  )}
                    ...
                    <span>
                      ...
                      {!testResult.canEarnAgain && testResult.nextAvailableDate && (
                        <span className='text-muted-foreground ml-1'>
                          (next available:{' '}
                          {new Date(testResult.nextAvailableDate).toLocaleDateString()})
                        </span>
                      )}
                    </span>
                  </div>
