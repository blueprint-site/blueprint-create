...
interface SchematicPreviewProps {
  formData: Partial<SchematicFormValues>;
}

export function SchematicPreview({ formData }: SchematicPreviewProps) {
  const { user } = useUserStore();

  // Extract values from formData with defaults
  const title = formData?.title || 'Schematic Title';
  const description = formData?.description || 'Schematic description will appear here';
  const categories = formData?.categories || [];
  const subCategories = formData?.sub_categories || [];
  const createVersions = formData?.create_versions || [];
  const gameVersions = formData?.game_versions || [];
  const modloaders = formData?.modloaders || [];
  const imagePreviewUrls = formData?.imageFiles
            ...
            <div>
              ...
              <div className='flex flex-wrap gap-2'>
                {categories && categories.length > 0 ? (
                  ...
                  <p className='text-foreground-muted text-sm'>No categories selected</p>
                )}
              </div>
            </div>
            <div>
              ...
              <div className='flex flex-wrap gap-2'>
                {subCategories && subCategories.length > 0 ? (
                  ...
                  <p className='text-foreground-muted text-sm'>No subcategories selected</p>
                )}
              </div>
            </div>
          </div>
